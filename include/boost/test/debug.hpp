...
///
/// Unfortunately these actions differ widely between different debuggers avaialble in a field. These interface present generalized standard form of 
...
///
/// All these interfaces are deined in namespace boost::debug
...
struct dbg_startup_info {
    ...
    unit_test::const_string init_done_lock;     ///< path to a uniquly named lock file, which is used to pause current application while debugger is being initialized
};
...
/// There are  many different debuggers avaialable for different platforms. Some of them also can be used in a different setups/configuratins.
...
/// @returns            Id of previosly selected debugger configuration
...
/// @param[in] break_or_continue tells what we wan to do aftr the debugger is attached. If true - process execution breaks
