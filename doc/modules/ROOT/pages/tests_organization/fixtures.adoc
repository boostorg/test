...
The {utf} lets you define a fixture according to xref:tests_organization/fixtures.adoc#models[several generic interfaces],
...
Several fixture interfaces are supported by the {utf}. The choice of the interface depends
mainly on the usage of the fixture.

=== Fixture class model

The {utf} defines the generic fixture class model as follows:
...
This is why the {utf} also supports (Boost 1.65 on) optional `setup` and/or `teardown` functions as follow:
...
As mentioned, the declaration/implementation of the `setup` and `teardown` are optional:
 the {utf} will check the existence of those and will call them adequately. However in C++98,
...
In addition to {boost_fixture_test_case} and {boost_fixture_test_suite} the {utf} allows to associate fixture with
test unit using the decorator {decorator_fixture}. This decorator supports additional models for declaring
...
For complete example of test module which uses these models please check decorator {decorator_fixture}.
...
The {utf} provides several ways of defining fixtures for test-cases, each of which having their properties:
...
* the use of the macro {boost_fixture_test_case} in place of {boost_auto_test_case}, which let access to the members of the fixture
* the use of the decorator {decorator_fixture}, which does not let access to the members but enables
...
the test case declaration with {boost_auto_test_case}:
...
The only difference from the macro {boost_auto_test_case} is the presence of an extra argument `fixture_name`.
...
By using the decorator {decorator_fixture}, it is possible to:
...
macro {boost_auto_test_suite} for automated test suite creation and registration.
...
Once again the only difference from the macro {boost_auto_test_suite} usage is the presence of
...
If necessary you can reset the fixture for a particular test case using the macro
    {boost_fixture_test_case}. Similarly you can reset the fixture for a particular sub
    test suite using {boost_fixture_test_suite}.
====

[NOTE]
====
The fixture assignment is _deep_. In other words unless reset by another
    {boost_fixture_test_suite} or {boost_fixture_test_case} definition the
...
The {utf} global fixture design is based on the
...
the {utf}. Introducing the initialization function can be more work than using the global fixture facility,
...
. and to pass the class as an argument to the macro {boost_test_global_fixture}.
