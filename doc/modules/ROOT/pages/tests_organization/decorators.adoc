...
it as the second argument to macro {boost_auto_test_case} or the third argument to macro {boost_fixture_test_case}.
...
has one associated _decorator:_ {decorator_label}. This means that when test units are filtered based on label, this
...
decorators as the second argument to the macro {boost_auto_test_suite} or the third argument to the macro
...
three test cases and associated with two {decorator_label} decorators. Therefore running tests by label `"trivial"` as
...
There is another way of associating a decorator set with test units. Macro {boost_test_decorator} indicates that its set
...
In the above example a decorator is applied to a xref:tests_organization/test_case_generation.adoc[data-driven test case]. Macro {boost_data_test_case} cannot take the decorator set as one of its arguments, therefore
the explicit decorator declaration is used. Macro {boost_data_test_case} generates a sequence of 4 test cases. The
