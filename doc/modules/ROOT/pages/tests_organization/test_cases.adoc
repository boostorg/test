...
xref:testing_tools/boost_test_universal_macro.adoc[assertions], and its execution is monitored by the {utf}.
...
The {utf} covers the following test case scenarios:
...
For this APIs, {utf} opted for a least intrusive design based on _generic callback_ approach, which signatures
...
[CAUTION]
====
The design of manual test case declaration API in {utf} assumes the test case implementation (test function body)
...
{decorator_enabled} _ {decorator_disabled} _ {decorator_enable_if} decorators instead
...
* If you need to specify complicated test unit dependencies, you can use {decorator_depends_on}
decorator instead
