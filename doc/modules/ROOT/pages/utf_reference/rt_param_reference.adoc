...
Following sections provide detailed specification for all {utf} runtime parameters. Each parameter specification includes:
...
The {utf} runtime parameters take value of the following types: string, boolean, enumeration,
...
Option _build_info_ instructs the {utf} to display the build information before testing begins.
...
If option _catch_system_errors_ has value "no" the {utf} does not attempt to catch asynchronous system failures
...
[NOTE]
====
If the {utf} detects that the current process is being run under a debugger, the `catch_system_errors` is
...
NOTE: The {utf} can be built with flag `BOOST_TEST_DEFAULTS_TO_CORE_DUMP` in which case default changes to *yes*
...
The {utf} is able to produce color output on systems which support it. To disable this behavior, set this option to
...
`testcase1` and `testcase3`. `testcase2` is not enabled by default, but can be enabled by using the parameter {param_run_test}.
...
Parameter _log_format_ allows to set the {utf} log format to one of the formats supplied by the framework.
...
Parameter _log_level_ allows to set the {utf} xref:test_output/logging_api.adoc#ref_log_level_explanations[log level]. Log level defines
...
Messages reporting user or system originated fatal errors. For example, memory access violation. Also
     all the messages describing failed assertion on `REQUIRE` level ({boost_test_require} / `BOOST_REQUIRE_...` tools)
...
Parameter _random_ instructs the {utf} to execute the test cases in random order. This parameter
...
[TIP]
====
in case `--random` is specified, the value of the seed is logged using {boost_test_message}, so that it is possible to replay
...
Parameter _report_format_ allows to set the {utf} report format to one of the formats supplied
...
the {utf}. Use value "no" to eliminate the results report completely. See the
...
The "no" argument value for the option \`result_code instructs the {utf} to always return zero
...
Parameter _run_test_ allows to filter which test units to execute during testing. The {utf} supports
both "selection filters", which allow to select which test units to enable from the set of available
test units, and "disabler filters", which allow to disable some test units. The {utf} also supports
...
Option _save_pattern_ facilitates switching mode of operation for testing output streams. See {output_test_stream_tool}
...
Option _show_progress_ instructs the {utf} to display test progress information. By default the
...
Option _use_alt_stack_ instructs the {utf} to use alternative stack for signals processing, on
...
If specified option _usage_ instructs the {utf} to displays short usage message about the
...
Option _wait_for_debugger_ instructs the {utf} to pause before starting test units execution,
