...
Creates a typed test case. The test case should have been declared with the macro xref:{boost_test_case_template_function}[`BOOST_TEST_CASE_TEMPLATE_FUNCTION`].
...
to the desired value *before* including the _Unit Test Framework_ headers.
...
xref:{boost_data_test_case}[`BOOST_DATA_TEST_CASE`] with fixture support added.
...
This macro is deprecated in favor of xref:{boost_test_global_fixture}[`BOOST_TEST_GLOBAL_FIXTURE`] and xref:{boost_test_global_configuration}[`BOOST_TEST_GLOBAL_CONFIGURATION`].
...
Sets the test unit's xref:{default_run_status}[_default run status_] to _true_ or _false_.
...
Sets the test unit's xref:{default_run_status}[_default run status_] to _true_ or _false_, depending on a compilation-time
constant.
See xref:tests_organization/enabling.adoc[here] for more details.
...
Associates a _predicate_ with a test unit that will determine its xref:{default_run_status}[_default run status_] at run-time.
See xref:tests_organization/enabling.adoc[here] for more details.
