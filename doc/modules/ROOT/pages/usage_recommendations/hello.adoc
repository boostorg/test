...
:page-aliases: boost_test/usage_recommendations/tutorials/hello.adoc
...
The _Unit Test Framework_ solves all these issues. To integrate with it above program needs to be changed to:
...
xref:{boost_test}[`BOOST_TEST`] tool, would you choose to see it. Is there any other ways to perform checks? The following example test
program shows several different ways to detect and report an error in the `add()` function.

[source,cpp]
----
include::example$snippets/snippet12.cpp[tag=snippet12]
----
include::example$snippets/snippet12.cpp[tag=snippet12-callouts]
