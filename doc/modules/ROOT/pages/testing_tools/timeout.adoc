...
The {utf} provides the decorator {decorator_timeout} that specifies a time-out for a specific *test unit*.
...
exceeded, the test case is reported as failed. On some systems, the {utf} is able to force the test-case
...
execution time needed by the {utf}. For each test-units under a test-suite with time-out, the maximum allowed duration
is set as being the test-suite time out minus the accumulated execution time before the execution of the test-unit.
If this test-unit is a test-case, it is equivalent to setting the decorator {decorator_timeout} to the test-case
...
The macro
  ...
  The {utf} will still be able to report the test-case as failed (once the test-case finishes).
====

NOTE: The support of test suite level time-out has been added in xref:change_log.adoc#ref_CHANGE_LOG_3_10[Boost 1.70 / {utf} v3.10]
