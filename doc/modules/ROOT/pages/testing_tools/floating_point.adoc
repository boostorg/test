...
Unless specified otherwise, when a value of floating-point type is compared inside a {boost_test} assertion,
...
xref:tests_organization/decorators.adoc[decorator] {decorator_tolerance}:
...
Manipulators requires a compiler that supports variadic macros, `auto` for type deduction
 ...
 lacking these features, resort to defining tolerance per test unit or to compatibility test assertions: {boost_check_close} and {boost_check_small}.
====
...
When values of two different floating point types `T` and `U` are compared, {boost_test} uses the tolerance
...
types `T` and `U` appear as sub-expressions of the full expression inside assertion {boost_test}. It is not applied when
...
[NOTE]
====
This behavior has been introduced in Boost 1.70 / {utf} xref:change_log.adoc#ref_CHANGE_LOG_3_10[3.10]. Previously tolerance based comparison was used only when the type of the two
...
The {utf} recognizes that a given type `T` is suitable for tolerance-based comparisons using the expression
...
Assertions in the {utf} use two kinds of comparison. For `u` being close to zero with absolute tolerance `eps`:
...
For rationale for choosing these formulae, see section {floating_points_testing_tools}.
...
The compatibility assertions {boost_level_close} and {boost_level_close_fraction} perform formula (rel).

The compatibility assertion {boost_level_small} performs formula (abs).

The {utf} also provides unary predicate cpp:boost::math::fpc::small_with_tolerance[`small_with_tolerance`] and binary predicate predicate
...
NOTE: This implies that the exactly one of these: `u < v`, `u == v`, `u > v`, passes with {boost_test} at any given tolerance.
...
The {utf} implements floating-point comparison algorithm that is based on the solution presented in xref:testing_tools/floating_point.adoc#KnuthII[Knuth]:
