...
input is causing an exception to be thrown use xref:{boost_level_throw}[`BOOST_<level>_THROW`] family of testing
tools.
...
The decorator xref:{decorator_expected_failures}[`expected_failures`] defines the number of assertions that are expected to fail within the corresponding test
...
xref:{boost_auto_test_case_expected_failures}[`BOOST_AUTO_TEST_CASE_EXPECTED_FAILURES`] footnote:[deprecated] before the test case definition.
...
test suite where xref:{boost_auto_test_case_expected_failures}[`BOOST_AUTO_TEST_CASE_EXPECTED_FAILURES`] resides.
