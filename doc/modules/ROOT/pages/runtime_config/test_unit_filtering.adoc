...
The _Unit Test Framework_ offers a number of ways to run only a subset of all test cases registered in the test tree.
...
xref:{decorator_disabled}[`disabled`], xref:{decorator_enabled}[`enabled`] and xref:{decorator_enable_if}[`enable_if`]. The default run status is set once, upon testing program
...
failed or skipped or disabled. This can be declared with decorator xref:{decorator_depends_on}[`depends_on`].
. Upon an arbitrary predicate. This can be declared with decorator xref:{decorator_precondition}[`precondition`].
...
xref:{param_run_test}[`run_test`] it is possible to alter the static pre-set in a number of ways:
...
....
xref:{param_run_test}[`run_test`]=test_1
....
a|
....
test_1
...
[subs="+macros,+attributes,+quotes"]
....
xref:{param_run_test}[`run_test`]=suite_1/suite_1/test_1
....
a|
....
...
[subs="+macros,+attributes,+quotes"]
....
xref:{param_run_test}[`run_test`]=suite_1/suite_2
xref:{param_run_test}[`run_test`]=suite_1/suite_2/*
....
a|
....
...
[subs="+macros,+attributes,+quotes"]
....
xref:{param_run_test}[`run_test`]=suite_1/test_1,suite_2
....
a|
....
suite_1/suite_2/test_1
...
[subs="+macros,+attributes,+quotes"]
....
xref:{param_run_test}[`run_test`]=suite_1/test_1:test_1
....
a|
....
suite_1/test_1
...
[subs="+macros,+attributes,+quotes"]
....
xref:{param_run_test}[`run_test`]=@L1
....
a|
....
test_1
...
[subs="+macros,+attributes,+quotes"]
....
xref:{param_run_test}[`run_test`]=suite_1/test*
....
a|
....
suite_1/test_1
...
[subs="+macros,+attributes,+quotes"]
....
xref:{param_run_test}[`run_test`]=suite_1/*_1
....
a|
....
suite_1/suite_1/test_1
...
[subs="+macros,+attributes,+quotes"]
....
xref:{param_run_test}[`run_test`]=\*/\*/test_2
....
a|
....
suite_1/suite_1/test_2
...
If test unit `B` depends on test unit `A` and test `B` is specified to be run by xref:{param_run_test}[`run_test`], `A` is also run, even
if it is not specified, and its failure may cause the execution of `B` to be skipped. Similarly, the failed check of
the xref:{decorator_precondition}[`precondition`] may cause the test selected test to be skipped.
...
argument xref:{param_run_test}[`run_test`], with the value using similar syntax as in the absolute specification, but preceded with
...
[subs="+macros,+attributes,+quotes"]
....
> test_program --xref:{param_run_test}[`run_test`]=!<absolute_spec>
....
|disabler
...
[NOTE]
====
While enabler additionally enables the upstream dependencies (introduced with decorator xref:{decorator_depends_on}[`depends_on`]),
